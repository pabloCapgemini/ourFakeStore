...
import { EmptyStoreAdapter, ErrorStoreAdapter, MockStoreAdapter, StoreAdapter } from './fake-store-api-client';
...
let storeRepo = new StoreRepository(adapter);
...
ipcMain.on("set-store-with-no-products", async () => {
  const emptyAdapter = new EmptyStoreAdapter();
  storeRepo = new StoreRepository(emptyAdapter);
  await initializeStore();
});

ipcMain.on("set-store-with-error", async () => {
  const errorAdapter = new ErrorStoreAdapter();
  storeRepo = new StoreRepository(errorAdapter);
  await initializeStore();
});

ipcMain.on("set-store-with-mock-products", async () => {
  const mockAdapter = new MockStoreAdapter();
  storeRepo = new StoreRepository(mockAdapter);
  await initializeStore();
});
